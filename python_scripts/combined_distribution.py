...
species1 = sys.argv[2]
species2 = sys.argv[3]
save_flag = int(sys.argv[4]) # extra flag to enable saving 1 and 0
...
# --- normalize initial ---
hist_combined_0 = hist_combined_0 / np.max(hist_combined_0)
...
def animate(i):
    ...
    if np.max(hist_combined) > 0:
        hist_combined = hist_combined / np.max(hist_combined)

    bin_centers = (bins[:-1] + bins[1:]) / 2

    ax.clear()
    #ax.plot(bin_centers_0, hist_combined_0, linestyle='--', color='gray', label=r"$\omega_{pe}t = 0$")
    #ax.plot(bin_centers, hist_combined, color='blue', label=rf"$\omega_{{pe}}t = {ts[i]:.2f}$")

    ax.plot(bin_centers_0, hist_combined_0, linestyle='--', color='black')
    ax.plot(bin_centers, hist_combined, color='blue')
    #plt.semilogy(bin_centers_0, hist_combined, label='Total f(v)')
    ...
    # --- save plots if flag is enabled ---
    if save_flag == 1 and i % 10 == 0:  # save every 10th frame
        fig.savefig(pjoin(path_fig, f"distribution_frame_{i:04d}.png"), dpi=150)
