...
j = int(t) * write_interval
print("max t can be:", NUM_TS // write_interval)

den_e_fixed = f[f"fielddata/den_electron/{j}"][:]
den_i_fixed = f[f"fielddata/den_ion/{j}"][:]

plt.figure(figsize=(8, 4))
plt.plot(x, den_e_fixed, label=r"$n_e$", color='blue')
plt.plot(x, den_i_fixed, label=r"$n_i$", color='orange')
plt.xlabel(r"$x$")
plt.ylabel("Density")
plt.legend()
