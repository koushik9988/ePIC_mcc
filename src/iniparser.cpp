...
std::tuple<std::string, int, double> INIParser::loadtypeextract(const std::string &position_init)
{
    ...
    if (position_init.rfind("dump+", 0) == 0)
    {
        // Strip "dump+" and reuse existing parser
        std::string rest = position_init.substr(5); // after "dump+"

        auto [type, n, amp] = loadtypeextract(rest);

        // Encode as dump+sin, dump+cos, etc.
        return {"dump+" + type, n, amp};
    }

    if (position_init.rfind("tanh+", 0) == 0)
    {
        // Strip "dump+" and reuse existing parser
        std::string rest = position_init.substr(5); // after "dump+"
